...
class TestRelu(unittest.TestCase):
    ...
    def test_forward_activation(self):
        ...
        self.assertTrue(np.allclose(self.model.activations['a0'], self.x))
        self.assertTrue(np.allclose(self.model.z['z1'], [[0.3,0.4],[0.1,0.2]]))
        self.assertTrue(np.allclose(self.model.activations['a1'], [[0.3,0.4],[0.1,0.2]]))
        self.assertTrue(np.allclose(self.model.z['z2'], [[0.43,0.5],[0.19,0.22]]))
        self.assertTrue(np.allclose(self.model.activations['a2'], [[0.48250714,0.51749286],[0.49250056,0.50749944]]))
        self.model.backprop(out, self.y, regularize=False)
