...
TEST(SegmentedDotProduct, DotProductSingleThreadedEx1) { //this is based on the Backprop example 1 from 589 HW4
  ...
  float *input = new float[batch_size]{0.13f, 0.42f};
  float *d_weights;
  float *d_input;
  float *d_product;
  float *d_bias;
...
TEST(ForwardPass, SingleThreadedDotProduct2Ex1_BATCH_SIZE_1) {
  ...
  for(int i = 0; i < 2; i+=1) {
    ...
    cudaDeviceSynchronize();
    dotProductSegmented<<<nBlocks, nThreads>>>(d_activations, d_model->weights[1], d_activations+(offsets[1]*batch_size), batch_size, model.layer_size[1], model.layer_size[1], model.layer_size[2], d_model->biases[1]);
    cudaDeviceSynchronize();
...
TEST(DeltaCalculations, SingleThreadCalculateDeltas_Ex1) {
  int nWorkers = 1;
  int nThreadsPerWorker = 1;
  int batch_size = 2;
  float input[2] = {0.13000f, 0.42f};
  ...
  for(int i = 0; i < 2; i+=batch_size) {
    ...
    softmaxSegmented<<<nWorkers, nThreadsPerWorker>>>(d_activations+(offsets[1]), batch_size, model->layer_size[2]);
    cudaMemcpy(activations, d_activations, activations_size*batch_size*sizeof(float), cudaMemcpyDeviceToHost);
    //verify that cross entropy loss is working correctly
    //backward pass
  }
}